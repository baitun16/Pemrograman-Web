		...
		<div class="card">
			...
			<div class="card-body">
				...
				<div class="form-group row">
					...
					<div class="col-sm-10">
						<input type="text" name="tgllhr" class="form-control <?php echo ($error_tgllhr !="" ? "is-invalid" : ""); ?>" id="tgllhr" placeholder="Tanggal Lahir (dd-mm-yyyy)" value="<?php echo $tgllhr; ?>"><span class="warning"><?php echo $error_tgllhr; ?></span>
					</div>
				</div>
				...
				<div class="form-group row">
					...
					<div class="col-sm-10">
						<input type="text" name="noregakta" class="form-control <?php echo ($error_noregakta !="" ? "is-invalid" : ""); ?>" id="noregakta" placeholder="Nomor Regristrasi Akta Lahir" value="<?php echo $noregakta; ?>"><span class="warning"><?php echo $error_noregakta; ?></span>
					</div>
				</div>
				...
				<div class="form-group row">
					...
					<div class="col-sm-10">
						<input type="text" name="alamatjl" class="form-control <?php echo ($error_alamatjl !="" ? "is-invalid" : ""); ?>" id="alamatjl" placeholder="Alamat Jalan" value="<?php echo $alamatjl; ?>"><span class="warning"><?php echo $error_alamatjl; ?></span>
					</div>
				</div>
				...
				<div class="form-group row">
					...
					<div class="col-sm-10">
						<input type="text" name="anke" class="form-control <?php echo ($error_anke !="" ? "is-invalid" : ""); ?>" id="anke" placeholder="Anak Ke" value="<?php echo $anke; ?>"><span class="warning"><?php echo $error_anke; ?></span>
					</div>
				</div>
				...
				<div class="form-group row">
					...
					<div class="col-sm-10">
						<input type="text" name="nopenerima" class="form-control <?php echo ($error_nopenerima !="" ? "is-invalid" : ""); ?>" id="nopenerima" placeholder="No Penerima" value="<?php echo $nopenerima; ?>"><span class="warning"><?php echo $error_nopenerima; ?></span>
					</div>
				</div>
...
if (!empty($tgl) && !empty($nama) && !empty($jk) && !empty($nisn) && !empty($nik) && !empty($tmptlhr) && !empty($noregakta) && !empty($agama) && !empty($kwn) && !empty($kebkhus) && !empty($alamatjl) && !empty($rt) && !empty($rw) && !empty($dsn) && !empty($desa) && !empty($kec) && !empty($kdpos) && !empty($domisili) && !empty($transport) && !empty($anke) && !empty($penerima)){
		...
		$sql="INSERT INTO maba (tgl, nama, jk, nisn, nik, tmptlhr, tgllhr, noregakta, agama, kwn, negara, kebkhus, alamatjl, rt, rw, dsn, desa, kec, kdpos, lintang, bujur, domisili, transport, nokks, anke, penerima, nopenerima)VALUES('$tgl', '$nama', '$jk', '$nisn', '$nik', '$tmptlhr', '$tgllhr', '$noregakta', '$agama', '$kwn', '$negara', '$kebkhus', '$alamatjl', '$rt', '$rw', '$dsn', '$desa', '$kec', '$kdpos', '$lintang', '$bujur', '$domisili', '$transport', '$nokks', '$anke', '$penerima', '$nopenerima')";

	if (mysqli_query($conn,$sql)) {
		echo "<script>alert('Data anda berhasil di simpan');window.location='siswa_baru.php'</script>";
	} else {
		
		echo "<h4>Maaf Data Anda Gagal Disimpan</h4>";
		echo "<hr>";
		echo "<a href='siswa_baru.php'>Kembali";	
	}
}
?>
...
include ('konek.php');
		...
		$sheet->setCellValue('I1', 'No Regristrasi Akta');
		$sheet->setCellValue('J1', 'Agama');
		$sheet->setCellValue('K1', 'Kewarganegaraan');
		$sheet->setCellValue('L1', 'Negara');
		$sheet->setCellValue('M1', 'Kebutuhan Khusus');
		$sheet->setCellValue('N1', 'Alamat Jalan');
		...
		$sheet->setCellValue('T1', 'Kode Pos');
		$sheet->setCellValue('U1', 'Lintang');
		$sheet->setCellValue('V1', 'Bujur');
		$sheet->setCellValue('W1', 'Tempat Tinggal');
		$sheet->setCellValue('X1', 'Moda Transportasi');
		$sheet->setCellValue('Y1', 'Nomor KKS');
		$sheet->setCellValue('Z1', 'Anak Ke');
		$sheet->setCellValue('AA1', 'Penerima KPS/PKH');
		$sheet->setCellValue('AB1', 'No KPS/PKH');
...
while($row = mysqli_fetch_array($query))
{
	...
	$sheet->setCellValue('G'.$i, $row['tmptlhr']);
	$sheet->setCellValue('H'.$i, $row['tgllhr']);
	$sheet->setCellValue('I'.$i, $row['noregakta']);
	$sheet->setCellValue('J'.$i, $row['agama']);
	$sheet->setCellValue('K'.$i, $row['kwn']);
	$sheet->setCellValue('L'.$i, $row['negara']);
	$sheet->setCellValue('M'.$i, $row['kebkhus']);
	$sheet->setCellValue('N'.$i, $row['alamatjl']);
	$sheet->setCellValue('O'.$i, $row['rt']);
	$sheet->setCellValue('P'.$i, $row['rw']);
	$sheet->setCellValue('Q'.$i, $row['dsn']);
	$sheet->setCellValue('R'.$i, $row['desa']);
	$sheet->setCellValue('S'.$i, $row['kec']);
	$sheet->setCellValue('T'.$i, $row['kdpos']);
	$sheet->setCellValue('U'.$i, $row['lintang']);
	$sheet->setCellValue('V'.$i, $row['bujur']);
	$sheet->setCellValue('W'.$i, $row['domisili']);
	$sheet->setCellValue('X'.$i, $row['transport']);
	$sheet->setCellValue('Y'.$i, $row['nokks']);
	$sheet->setCellValue('Z'.$i, $row['anke']);
	$sheet->setCellValue('AA'.$i, $row['penerima']);
	$sheet->setCellValue('AB'.$i, $row['nopenerima']);	
	$i++;
}
...
$i = $i - 1;
$sheet->getStyle('A1:AB'.$i)->applyFromArray($styleArray);


$writer = new Xlsx($spreadsheet);
$writer->save('Report Siswa Baru.xlsx');
?>